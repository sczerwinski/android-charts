...
class LongListPieChartAdapterTest {
    ...
    fun `When constructor, then create adapter with initial data`() {
        val initialData = listOf(1L, 2L, 3L)
        val adapter = LongListPieChartAdapter(context, initialData)
    ...
    fun `When setData, then update data and notify observers`() {
        val initialData = listOf(1L, 2L, 3L)
        val updatedData = listOf(4L, 5L, 6L)
        val adapter = spyk(LongListPieChartAdapter(context, initialData), recordPrivateCalls = true)
