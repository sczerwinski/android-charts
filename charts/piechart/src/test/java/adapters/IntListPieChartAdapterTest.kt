...
class IntListPieChartAdapterTest {
    ...
    fun `When constructor, then create adapter with initial data`() {
        ...
        val adapter = IntListPieChartAdapter(context, initialData)
    ...
    fun `When setData, then update data and notify observers`() {
        val initialData = listOf(1, 2, 3)
        val updatedData = listOf(4, 5, 6)
        val adapter = spyk(IntListPieChartAdapter(context, initialData), recordPrivateCalls = true)
