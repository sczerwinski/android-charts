...
class DoubleListPieChartAdapterTest {
    ...
    fun `When constructor, then create adapter with initial data`() {
        val initialData = listOf(1.0, 2.0, 3.0)
        val adapter = DoubleListPieChartAdapter(context, initialData)
    ...
    fun `When setData, then update data and notify observers`() {
        val initialData = listOf(1.0, 2.0, 3.0)
        val updatedData = listOf(4.0, 5.0, 6.0)
        val adapter = spyk(DoubleListPieChartAdapter(context, initialData), recordPrivateCalls = true)
